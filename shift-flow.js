(() => {
  ...
  function showShiftModal(old = null) {
    const locations = (window.cache?.locations || []).filter(l => l.active !== false);
    ...
    $('shiftFlowForm').onsubmit = async (event) => {
      ...
      try {
        ...
        const row = {
          date, start_time:startTime, location_id:location.id, location_name:location.name,
          duration, value, value12:value, notes:String(f.get('notes') || '').trim() || null,
          status:String(f.get('status') || 'scheduled')
        };

        if(old) {
          const { error } = await db.from('shifts').update(row).eq('id', old.id).eq('user_id', user.id);
          if(error) throw error;
          const { error: receivableError } = await db.from('receivables').update({location_id:row.location_id,description:`Plantão · ${row.location_name}`,amount:row.value}).eq('shift_id',old.id).eq('user_id',user.id).neq('status','received');
          if(receivableError) throw receivableError;
        } else {
          const shiftId = crypto.randomUUID();
          row.id = shiftId; row.user_id = user.id;

          const { error: shiftError } = await db.from('shifts').insert(row);
          if(shiftError) throw shiftError;

          const expected = new Date(`${date}T12:00:00`);
          expected.setDate(expected.getDate() + 30);
          const { error: receivableError } = await db.from('receivables').insert({
            user_id:user.id, shift_id:shiftId, location_id:location.id,
            description:`Plantão · ${location.name}`, amount:value,
            expected_date:expected.toISOString().slice(0,10), status:'pending'
          });
          if(receivableError) {
            await db.from('shifts').delete().eq('id',shiftId).eq('user_id',user.id);
          ...
          if(f.get('repeat') === 'on') {
            ...
            if(!occurrences && !endDate) {
              await db.from('receivables').delete().eq('shift_id',shiftId).eq('user_id',user.id);
              await db.from('shifts').delete().eq('id',shiftId).eq('user_id',user.id);
              throw new Error('Para repetir o plantão, informe a data final ou a quantidade de ocorrências.');
            }
            ...
            const { error: recurrenceError } = await db.from('recurrences').insert({
              id:recurrenceId, user_id:user.id, frequency, interval_value:interval,
              start_date:date, end_date:endDate, occurrences, active:true
            });
            if(recurrenceError) {
              await db.from('receivables').delete().eq('shift_id',shiftId).eq('user_id',user.id);
              await db.from('shifts').delete().eq('id',shiftId).eq('user_id',user.id);
            ...
            const { error: linkError } = await db.from('shifts').update({recurrence_id:recurrenceId}).eq('id',shiftId).eq('user_id',user.id);
        ...
        save.disabled = false;
        save.textContent = old ? 'Salvar alterações' : 'Adicionar plantão';
      }
    };
  }

  window.openForm = function(type, id='', spaceId='') {
    if(type === 'shift') return showShiftModal(id ? (cache.shifts || []).find(s => s.id === id) : null);
    return originalOpenForm(type, id, spaceId);
  };

  ['newShiftButton','mobileNewShift','addShiftTop'].forEach(id => {
    const button = $(id);
    if(button) button.onclick = () => showShiftModal();
  });
})();
